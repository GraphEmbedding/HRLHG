/*
Reference implementation of metapath2vec for heterogeneous node embedding in citation network.
For citation network, there are two node type: paper, topic(p,t).
Author: Yue Yin
*/
...
const int vocab_hash_size = 300000000;  // Maximum 240 * 0.7 = 168M words in the vocabulary

const int table_size = 3e8;
...
std::vector<long long> p_table(0), t_table(0);
...
void NodeType() {
  ...
  for (i = 0; i < table_size; i++) {
    ...
    if (vocab[target].word[0] == 'p') {
      p_table.push_back(target);
    } else if (vocab[target].word[0] == 't') {
      t_table.push_back(target);
    }
  }
  //std::cout << "table_size     " << table_size << std::endl;
  //std::cout << "p_table.size() " << p_table.size() << std::endl;
  //std::cout << "t_table.size() " << t_table.size() << std::endl;
}
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //
          if (d == 0) {
            ...
            if (pp) {
              if        (vocab[last_word].word[0] == 'p') {
                target = p_table[(next_random >> 16) % p_table.size()];
              } else if (vocab[last_word].word[0] == 't') {
                target = t_table[(next_random >> 16) % t_table.size()];
              }
